...
export async function authenticate(request) {
    ...
    if(request.headers.authorization != null && request.headers.authorization.startsWith("Basic ")) {
        ...
        if(!credential.includes(":") || credential.indexOf(":") == credential.length-1) {
        ...
        password = credential.substring(credential.indexOf(":")+1);
...
export async function authorize(request) {
    ...
    if(jwtBody.exp <= current) {
        let transaction = await connection.transaction();
        try {
            await transaction.delete().from("UserLogin").where({UserId: jwtBody.sub, Session: jwtBody.jti});
            await transaction.commit();
        }catch(error) {
            await transaction.rollback();
            throw error;
    ...
    let session = {userId: jwtBody.sub, organizationId: jwtBody.org, sessionId: validSession.session};

    verifyHttpSignature(session, request);
 ...
 */
async function verifyHttpSignature(session, request) {
    let organization = await getOrganization(session.organizationId);
    if(organization == null) {
        throw ErrorResponse(ErrorCode.RequestError, JSON.stringify({code: "BadRequest", message: "Invalid state."}));
    }
